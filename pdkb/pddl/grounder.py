...
class GroundProblem(Problem):
    ...
    def _predicate_to_fluent(self, predicate, assignment, fluent_dict={}):
        ...
        fluent.always_known = predicate.always_known
    ...
    def _action_to_operator(self, action, assignment, fluent_dict):
        ...
        op_name_suffix = "_".join([assignment[var_name] for var_name, _ in action.parameters])
        if op_name_suffix:
            op_name = action.name + "_" + op_name_suffix
        else:
            op_name = action.name
    ...
    def _ground_goal(self, fluent_dict):
        """Ground the goal state."""

        d = {}
        #self._get_unground_vars(self.init, d)
        self.goal = self._partial_ground_formula(self.goal, d, fluent_dict)
...
class Operator(Action):
    ...
    def __init__(self, name, parameters, precondition, observe, effect, dcond):
        ...
        super(Operator, self).__init__\
            (name, parameters, precondition, observe, effect, dcond)
